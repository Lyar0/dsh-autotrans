...
**随使用不断累积的本地词表库。**校订/自动词对还会持久化累进一个位于
`$DSH_HOME/autotrans/user_glossary.tsv` 的用户级词表（独立运行时 `DSH_HOME` 默认
`~/.dsh`）。每个新增/自动词对只会并入一次；该库的内容会并入你**之后所有**翻译，
而已存在的词对不会被后来的自动词表覆盖。用越久，本地术语越全、越统一，且跨不同
研究领域都能沿用。你也可以直接手改这个 TSV（每行 `英文<TAB>中文`）——你的手改译名
优先级最高、压过自动词。该库放在插件目录之外，重装插件不会丢失。
