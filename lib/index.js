...
const PARAMETERS = {
  ...
  properties: {
    ...
    mode: {
      ...
      enum: ["cn", "bilingual"],
      description:
        "Render layout. 'cn' (default) outputs Chinese only; 'bilingual' outputs each paragraph as one English paragraph followed by its Chinese translation (interleaved), so you can proofread side by side.",
    },
    api_key: {
      type: "string",
      description:
  ...
  const tool = {
    ...
    async execute(args, exec) {
      ...
      const argv = frozenRuntime
        ? [action, "--out-dir", outDir, "--model", model, "--concurrency", String(concurrency), "--format", format]
        : [CLI, action, "--out-dir", outDir, "--model", model, "--concurrency", String(concurrency), "--format", format];
      if (args.mode === "bilingual") argv.push("--mode", "bilingual");
