...
/**
 * True when `p` points inside a Zotero data directory's `storage/` tree.
 *
 * Zotero's storage layout is `<data-dir>/storage/<8-char item key>/<attachment>`, and
 * Zotero watches that tree and holds handles on its attachments, so rendered output
 * placed there tends to stay locked and unopenable in Word. Matched either by the
 * well-known `Zotero/storage` shape (any drive, any Zotero data dir) or by the key
 * folder shape alone, so a relocated Zotero data directory is still caught.
 */
function isZoteroStorage(p) {
  const norm = String(p || "").replace(/\\/g, "/");
  return /\/zotero\/storage\//i.test(norm) || /\/storage\/[A-Z0-9]{8}(\/|$)/.test(norm);
}
...
const PARAMETERS = {
  ...
  properties: {
    ...
    mode: {
      ...
      enum: ["cn", "bilingual"],
      description:
        "Render layout. 'bilingual' (default) interleaves each English paragraph with its Chinese " +
        "translation so the two can be checked side by side; 'cn' outputs Chinese only.",
    },
    api_key: {
      type: "string",
      description:
    ...
    allow_zotero_out: {
      type: "boolean",
      description:
        "Set true only to override the Zotero guard. Zotero continuously watches its storage/ directory, " +
        "which locks the files it holds, so a DOCX written there often cannot be opened in Word.",
    },
  },
  required: ["pdf_path"],
};
...
export function apply(ctx, config = {}) {
  ...
  const defaults = {
    ...
    // Default layout is bilingual (each English paragraph followed by its Chinese
    // translation); 'cn' renders Chinese only.
    mode: config.mode === "cn" ? "cn" : "bilingual",
  };
  ...
  const tool = {
    ...
    async execute(args, exec) {
      ...
      // Refuse to write into Zotero's managed storage. Zotero watches storage/ and keeps
      // a lock on the files it holds, so a DOCX written there frequently cannot be
      // opened in Word ("file is locked / already in use"). Output belongs in the
      // workspace; the user can copy it out afterwards if they want it elsewhere.
      if (isZoteroStorage(outDir) && args.allow_zotero_out !== true) {
        return {
          ok: false,
          message:
            `autotrans: refusing to write into Zotero's storage directory (${outDir}). ` +
            "Zotero keeps files under storage/ locked, so the rendered DOCX usually cannot be " +
            "opened in Word — the translation itself does not fail, only the file stays locked. " +
            "Use a workspace path instead (e.g. " + join(workspace, "autotrans") + "), or pass " +
            "allow_zotero_out: true to override.",
          out_dir: "",
          files: [],
          stdout: "",
          stderr: "",
      ...
      // Arguments, not the script path: the packaged runtime is a frozen executable
      // that already carries autotrans.py, and passing the script path as a positional
      // argument makes argparse read it as `action` and complain about unknown actions.
      // Only a real interpreter (fallback `python`) needs the script path as argv[1].
      const frozenRuntime = /autotrans\.exe$/i.test(python);
      // The PDF path travels in AUTOTRANS_PDF rather than argv: on Windows the frozen
      // runtime's argument parsing decodes the command line as ANSI, so a non-ASCII
      // (e.g. CJK) file name arrives mangled and argparse reports "unrecognized
      // arguments". Environment variables cross that boundary intact.
      // Layout follows config/argument, and is always passed explicitly so the rendered
      // document matches what the caller asked for instead of an implicit default.
      const mode = args.mode === "cn" || args.mode === "bilingual" ? args.mode : defaults.mode;
      const argv = frozenRuntime
        ? [action, "--out-dir", outDir, "--model", model, "--concurrency", String(concurrency), "--format", format, "--mode", mode]
        : [CLI, action, "--out-dir", outDir, "--model", model, "--concurrency", String(concurrency), "--format", format, "--mode", mode];
      ...
      const extraEnv = { AUTOTRANS_PDF: pdfPath };
