 ...
 * /`stderr` are returned.
 */
function runPython(python, argv, extraEnv, signal) {
...
export function apply(ctx, config = {}) {
  const python = typeof config.python === "string" && config.python ? config.python : "python";
  const defaults = {
  ...
  const tool = {
    ...
    async execute(args, exec) {
      ...
      const argv = [CLI, action, pdfPath, "--model", model, "--concurrency", String(concurrency), "--format", format];
      const push = (flag, value) => {
        if (value !== undefined && value !== null && value !== "") argv.push(flag, String(value));
      };
      push("--out-dir", args.out_dir);
      push("--glossary", args.glossary);
      push("--config", args.config_path);

      const extraEnv = {};
      if (typeof args.api_key === "string" && args.api_key.trim()) {
        extraEnv.DEEPSEEK_API_KEY = args.api_key.trim();
      }

      const result = await runPython(python, argv, extraEnv, exec?.signal);
